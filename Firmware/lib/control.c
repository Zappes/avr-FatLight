...
void handle_command(char* commandBuffer) {
	...
	if (strcmp(commandBuffer, "MODE") == 0) {
		...
		uart_writeln_string("Enabling strip");
		strip_switch_on();
	} else if (strcmp(commandBuffer, "OFF") == 0) {

		uart_writeln_string("Disabling strip");
		strip_switch_off();
		...
		uart_writeln_formatted("Current RGB : %06lX",
				anim_get_current_rgb_numeric());
		uart_writeln_formatted("Target RGB  : %06lX",
				anim_get_target_rgb_numeric());
		...
		uart_writeln_formatted("Current slot: %02X",
				persistence_get_current_slot());
		uart_writeln_formatted("Strip active: %02X", strip_is_on());
